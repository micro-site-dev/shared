...
export function useStore<S>(key: string) {
  const store = globalThis['stores'].get(key) as Store<S, AnyAction>
  return koala<S>(store)
}

export function addToStore<S>(key: string, store: Store<S, AnyAction>) {
  return globalThis['stores'].set(key, store).get(key) as Store<S, AnyAction> 
} 

globalThis['koalaStore'] = { addToStore, useStore, koala }